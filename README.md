...
给热爱 vibe coding 的人用的桌面工具台。SpringCat 不替代 Cursor、Codex、Grok 这些对话窗口，而是帮这群人更快上手常用 AI 编程工具：接上监听、改对 Key 和供应商，并在长任务、多窗口切换里把进度、等待、完成和用量收进同一个低干扰入口。

任务监听是第一项已交付能力；供应商 / Key 管理是下一刀要做的上手能力。这些能力都在 SpringCat 里原生实现，不嵌外部工具。

**官网：** [springcat.cn](https://springcat.cn)

> 当前版本为 `0.1.0`，工作面板已经实现；宠物模式仍在规划中，设置页暂不可选。产品定位与约束见 [项目描述](./docs/项目描述.md)。

## 主要功能

- **桌面工具台入口**：透明无边框、可置顶，可吸附到屏幕顶部、左侧或右侧；支持悬浮球、状态胶囊和最近任务抽屉。常驻形态只提示当前最重要的一件事。
- **更容易上手**：在设置页一键安装、修复、移除并测试 Codex、Cursor、Grok CLI、Gemini CLI、WorkBuddy 与 Marvis 等来源；启动时检查并修复已启用的绑定，不覆盖其他工具已有 hook。下一刀会在同一设置入口里管理 Codex / Gemini CLI / Grok CLI 的供应商和 API Key，一键写回本机配置。
- **过程中看得见**：汇总各工具的开始、进度、等待、完成、失败和取消；离开对话窗口后仍能及时回到该处理的那一件事。
- **低干扰通知策略**：支持未读标记、连续完成合并、静音 1 小时和专注模式；可在任务仍在执行时自动临时置顶。
- **回到现场**：本地保存最近任务，支持标记已读、打开任务 deep link，或回到对应的来源应用。
...
- 完整对话、推理、工具参数与结果、项目源代码不会写入 SpringCat。
- 供应商 API Key 只保存在本机供应商库，切换时写入对应工具配置；不进入任务事件、inbox 或用量库。云端备份是后续可选能力，默认关闭。
