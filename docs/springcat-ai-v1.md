...
> 产品名：`springcat-ai`  
> 本版目标：按步骤实现已写入规格的功能；**先做工作模式，宠物模式后置**

本文是实现手册，不是产品愿景文档。产品定位以 [项目描述.md](./项目描述.md) 为准：SpringCat 是 vibe coding 桌面工具台，任务监听是第一项能力，不是全部。每完成一步，勾选该步的任务和验收，再进入下一步。不要跳步，尤其不要在工作模式跑通前做 Rive / 宠物形象。未写入项目描述的新能力，不要直接加进本表。
...
- 不默认创建三个长期 WebView。
- 不把完整对话、源代码、Cookie 写入任务库或 inbox。供应商 API Key 只进本机供应商库，不进任务事件。
- 不把外部桌面应用嵌进 SpringCat；同类能力原生实现。
...
| ID | 功能 | 步骤 | 状态 |
|---|---|---|---|
| F60 | 本机供应商库：为 Codex / Gemini CLI / Grok CLI 保存多套名称、Base URL、API Key | 待拆步骤 | 未开始 |
| F61 | 一键切换供应商，原子写入目标配置并保留 SpringCat hooks | 待拆步骤 | 未开始 |
| F62 | 切换前备份、失败回滚；界面遮罩 Key | 待拆步骤 | 未开始 |
| F63 | 托盘快速切换当前供应商 | 待拆步骤 | 未开始 |

云端备份（设置、供应商配置、任务元数据、用量汇总；Key 加密、默认关闭）后置于 Key 管理可用之后，先不编号实施。

### 1.4 后续候选（v1 不做也可列出，便于以后开分支）
